...
var sum = function(array) {
  ...
  return array[0] === array[array.length - 1] ? array[array.length - 1] : array[0] + sum(array.slice(1), array[array.lenth - 1]);
};
// //!!!
...
var sumBelow = function(n) {
  if(Math.abs(n) === n) return n ? n - 1 + sumBelow(n - 1) : 0;
  
  if(n === -1) return 0;
  
  return Math.abs(n) ? -(Math.abs(n) - 1 + sumBelow(Math.abs(n) - 1)) : 0;
};
...
var numToText = function(str, result = '') {
  ...
  numberStrings[str[0]] ? result += numberStrings[str[0]] : result += str[0];
